"""The BIP-39 English wordlist: the 2,048 words a wallet recovery phrase is written in.

Kept here so a recovery phrase can be recognised, and redacted, by any process that
installs this library, none of which carries a wallet library. The list is the one
published with BIP-39; ``tests/test_job_errors.py`` pins it by count and by the
SHA-256 of the published file, so an edited or truncated copy fails.

Nothing here generates, stores or checks a phrase. It is a vocabulary, not a wallet.
"""

from __future__ import annotations

# In the published order, which the checksum covers.
_TEXT = """
...
WORDS: tuple[str, ...] = tuple(_TEXT.split())
